...
class Notification {
	...
	public function getNotifications($data, $limit) {
		...
		if($count > $limit)
			...
			$return_string .= "<input type='hidden' class='noMoreDropdownData' value='true'> <p style='text-align: center;'>אין יותר התראות</p>";
	...
	public function insertNotification($post_id, $user_to, $type) {
		...
		switch($type) {
			case 'comment':
				$message = $userLoggedInName . " הגיב על לך הפוסט";
				break;
			case 'like':
				$message = $userLoggedInName . " אהב את הפוסט";
				break;
			case 'profile_post':
				$message = $userLoggedInName . " הגיב לך בפרופיל";
				break;
			case 'comment_non_owner':
				$message = $userLoggedInName . " הגיב בפוסט שהגבתה עליו";
				break;
			case 'profile_comment':
				$message = $userLoggedInName . " הגיב בפוסט פרופיל שלך";
				break;
		}
